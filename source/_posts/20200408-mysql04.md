...
<h2>连接分析</h2>
    select * from table1 t1 xxx join table2 t2 on t1.id = t2.t1_id where t1.id=1<br/>
...
<h2>优化2（基于成本的优化）</h2>
    ...
    成本常数：类似于普朗克常量这种常数系数，一般是根据大量的实验验算出来的，mysql中规定读取一个页的成本默认为1.0，读取或者
            检测一条记录是否符合搜索条件默认成本为0.2.
    单表查询成本：
        过程:1.找出所有可使用索引
            2.计算全表扫描代价
...
type:<br/>
    system:单表使用的储存引擎统计数据是精确的，访问方法就是system（Myisam,memory）<br/>
    const，eq_ref,ref_or_null,index_merge，range，index，ALL等访问方法之前都有讲过<br/>
    fulltext：全文索引<br/>
    unique_subquery：优化器将in子查询转换成EXISTS子查询，且在执行子查询时会使用id列索引<br/>
    index_subquery：同unique_subquery，只不过执行子查询时使用普通二级索引<br/>

Extra：<br/>
    No tables used: 查询语句中没有from子句<br/>
    Impossible WHERE： where子句条件失效即为false<br/>
    No matching min/max row：查询列表处有MIN或者MAX聚集函数，但是并没有符合WHERE子句中的搜索条件的记录<br/>
    Using index：查询列和搜索条件中只包含属于某个索引的列，即索引覆盖<br/>
    Using index condition：索引条件下推情况，多个索引其中有些索引无法使用时会根据能使用到的索引先查询记录，然后用使用不到索引的条件过滤一遍再回表<br/>
    Using where：使用有where条件额全表扫描情况下<br/>
    Using join buffer (Block Nested Loop)：被驱动表无法有效使用索引时采用join buffer加快检索速度<br/>
    not exists：外连接查询时，检索被驱动表的条件无效，即无需查询被驱动表<br/>
    sing intersect(key...)：intersect索引合并<br/>
    Using union(key...)：union索引合并<br/>
    Using sort_union(key...)：sort_union索引合并<br/>
    Zero limit:limit参数为0<br/>
    Using filesort：无法使用索引排序，使用普通字段进行排序，一般是在内存或者磁盘中进行<br/>
    Using temporary：无法使用索引进行去重或者排序等操作，会建立临时表查询<br/>
    Start temporary, End temporary: IN子查询优化时使用DuplicateWeedout策略<br/>
    LooseScan：IN子查询时使用LooseScan策略<br/>
    FirstMatch：IN子查询转换为半连接时使用FirstMatch策略<br/>
    
结语：EXPLAIN FORMAT=JSON 可以将执行结果json化
<h2>内部优化过程（optimizer trace）</h2>  
 追踪优化日志方法：
```
